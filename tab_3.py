...
from generate_report_tab3 import generate_report

def display_form_url(base, path):
    full_url = "/".join([base, path])
    st.image(full_url)

def render_tab3(specific_specie_data_2, df_information, df_images):
    ...
    with col9:
        ...
        with st.container(border = True):
            ...
            st.text(f"- IUCN: {specific_specie_data_2['iucn'] or 'Không'}")
        ...
        image_data = df_images[['common_character', 'image', 'sp_1', 'genus']]
    ...
    with col10:  
        ...
        max_year_3 = pd.Timestamp.now().year
